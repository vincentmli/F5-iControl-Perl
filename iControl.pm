...
sub new {
    ...
    $self->{port} .= $arguments{port}
      || ( $arguments{protocol} && $arguments{protocol} eq 'https'
        ? DEFAULT_HTTPS_PORT
        : DEFAULT_HTTP_PORT );
    my $proxy_uri .= $arguments{proxy_uri} || DEFAULT_PROXY_URI;
    my $proxy =
"$self->{protocol}://$authentication$self->{host}:$self->{port}/$proxy_uri";
...
sub check_error {
    ...
    if ( defined( $params{fault_obj} ) ) {
        ...
        $display_message = $internal_error =
            $params{fault_obj}->faultcode . ": "
          . $params{fault_obj}->faultstring;
